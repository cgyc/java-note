- 配置个人的用户名称和电子邮件地址：
	+ $ git config --global user.name "chengy"
	+ $ git config --global user.email chengy@qq.com
...
git remote add origin https://gitee.com/share89/jingli-api.git
git push -u origin master /* 把本地仓库分支master内容推送到元仓库去，第一次才加上-u参数*/
